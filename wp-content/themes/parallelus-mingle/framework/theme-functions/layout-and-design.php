...
if ( ! function_exists( 'create_theme_layout' ) ) :
	...
	function create_theme_layout($source = 'default', $template = false, $layout_key = false) {
		...
		if (!is_array($layouts) || count($layouts) <= 0 ) {
			...
			$context_item = (in_array($context, array('category', 'tag', 'taxonomy')))? 'taxonomy' : $context;

			switch ($context_item) {
				case 'taxonomy':
 	 				$template_id = $context.'-'.get_queried_object()->term_id;
	 				$template_slug = $context.'-'.get_queried_object()->slug;
					break;

				default:
	 				$template_id = $context.'-'.$post->ID;
	 				$template_slug = $context.'-'.$post->post_name;
					break;
			}
			...
			$designTemplate = array(
				...
				'design.php' );
			if( ($custom_template = pathinfo(get_page_template(), PATHINFO_BASENAME)) != 'page.php')
				array_unshift($designTemplate, $custom_template);
	...
	if ( ! function_exists( 'page_layout_v1_compatibility' ) ) :
		...
		function page_layout_v1_compatibility($source = 'post', $template = false, $layout_key = false) {
			...
			$values = false;
			if ( $file = locate_template($template,false,false) ) {
...
if ( ! function_exists( 'display_slideShow' ) ) :
	...
	function display_slideShow($id, $width = false, $height = false) {
		...
		if ( is_array($ss) ) {
			...
			$speed = (isset($ss['speed']) && !empty($ss['speed'])) ? $ss['speed'] : '1000';		// speed of transition
			$pauseOnHover = (isset($ss['pause_on_hover']) && !empty($ss['pause_on_hover']) && $ss['pause_on_hover'] == true) ? '1' : '0'; // pause when mouse over
...
if ( ! function_exists( 'make_theme_portfolio_page' ) ) :
	...
	function make_theme_portfolio_page($settings = false) {
		...
		if ($settings && is_array($settings)) {
			...
			if ($portfolio['post_type'] == 'page' && !isset($portfolio['orderby']) ) {
...
if ( ! function_exists( 'make_theme_contact_form' ) ) :
	...
	function make_theme_contact_form($form = false) {
		...
		foreach ((array) $fields as $field) {
			...
			$values = isset($field['values']) ? explode(',', $field['values']) : array();
			...
			// the default label
			$defaultLabel = (isset($field['label'])) ? '<label for="'.$field_id.'" class="'.$labelClass.'">'.$field['label'].'</label>' : '';
			// the default caption
			$defaultCaption = (isset($field['caption'])) ? '<div class="formCaption">'.prep_content($field['caption'], 1, 0).'</div>' : '';
