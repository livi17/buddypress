...
/*
	BuddyPress can now be applied as a filter on the main template context function 
*/

if ( ! function_exists( 'bp_template_context' ) ) :
	function bp_template_context($context) {

		/**
		 *
		 *	Possible BP context values
		 *	--------------------------------
		 * 
		 *	bp-activity
		 *	bp-blogs
		...
		*/
		...
		// See if we have access to the BP page settings
		if (function_exists('bp_core_get_directory_page_ids')) {

			// Get the list of configured Page ID's for different BP areas (could be a problem with BP add-on plugins?)
			$bp_pages = bp_core_get_directory_page_ids();

			// Current page ID
			$current_page_id = (int) get_queried_object_id();

			// See if this is a BP page
			if (isset($current_page_id) && isset($bp_pages) && in_array($current_page_id, $bp_pages)) {
				
				// Set the default
				$context = 'bp';

				// Try and get more specific
				foreach ( $bp_pages as $name => $id ) {
					if ( $id == $current_page_id ) {
						
						// Set the context for the BP area
						$context = 'bp-'.$name;

						// Dig a little deeper and try to set a more specific context if possible
						if ( $context == 'bp-groups' && function_exists('bp_is_group_single') && bp_is_group_single() ) {
							 $context = 'bp-groups-single';
						}
						if ( $context == 'bp-members' ) {
							if(bp_get_version() > 1.5) {
								if ( function_exists('bp_is_user') && bp_is_user() )
									$context = 'bp-members-single';
							}
							elseif ( function_exists('bp_is_member') && bp_is_member() ) {
									$context = 'bp-members-single';
							}
						}
						// if ( $context == 'bp-groups' && function_exists('bp_is_group_single') && bp_is_group_single() ) {
						// 	 $context = 'bp-groups-single';
						// }
						// if ( $context == 'bp-members' && function_exists('bp_is_member') && bp_is_member() ) {
						// 	 $context = 'bp-members-single';
						// }
	...
	if (!is_admin()) {
		...
		add_filter( 'template_context', 'bp_template_context' );
	}
endif;
...
if ( ! function_exists( 'bbpress_context' ) ) :
	function bbpress_context($context, $template, $values) {
		...
		if ( is_bbpress() && $post->post_type != 'bp_members') {
