			...
			<div class="inside">
				...
				<table class="form-table">
					...
					<tfoot>
						<tr>
						<th colspan="2">
							<p>To modify these settings click the "Edit All Default Design Settings" button below.</p> 
							<?php
	...
	if (!empty($slideshows) || !empty($saved_slideshows)) {
		...
		if (!empty($saved_slideshows)) {
			foreach ($saved_slideshows as $key => $item) {
				...
				if (!$class && isset($_data['_plugin']['slideshows'])) $class = (array_key_exists($key, $_data['_plugin']['slideshows'])) ? 'disabled' : false ;
	...
	if (!empty($top_graphics) || !empty($saved_top_graphics)) {
		...
		if (!empty($saved_top_graphics)) {
			foreach ($saved_top_graphics as $key => $item) {
				...
				// Is this overwritten?
				$class = (in_array($key, $ancestor_keys)) ? 'disabled' : false;
				if (!$class && isset($_data['_plugin']['top_graphics'])) $class = (array_key_exists($key, $_data['_plugin']['top_graphics'])) ? 'disabled' : false ;
	...
	if (!empty($sidebars) || !empty($saved_sidebars)) {
		...
		if (!empty($saved_sidebars)) {
			foreach ($saved_sidebars as $key => $item) {
				...
				// Is this overwritten?
				$class = (in_array($key, $ancestor_keys)) ? 'disabled' : false;
				if (!$class && isset($_data['_plugin']['sidebars'])) $class = (array_key_exists($key, $_data['_plugin']['sidebars'])) ? 'disabled' : false ;
	...
	if (!empty($page_headers) || !empty($saved_page_headers)) {
		...
		if (!empty($saved_page_headers)) {
			foreach ($saved_page_headers as $key => $item) {
				...
				// Is this overwritten?
				$class = (in_array($key, $ancestor_keys)) ? 'disabled' : false;
				if (!$class && isset($_data['_plugin']['page_headers'])) $class = (array_key_exists($key, $_data['_plugin']['page_headers'])) ? 'disabled' : false ;
	...
	if (!empty($page_footers) || !empty($saved_page_footers)) {
		...
		if (!empty($saved_page_footers)) {
			foreach ($saved_page_footers as $key => $item) {
				...
				// Is this overwritten?
				$class = (in_array($key, $ancestor_keys)) ? 'disabled' : false;
				if (!$class && isset($_data['_plugin']['page_footers'])) $class = (array_key_exists($key, $_data['_plugin']['page_footers'])) ? 'disabled' : false ;
	...
	if (!empty($layouts) || !empty($saved_layouts)) {
		...
		if (!empty($saved_layouts)) {
			foreach ($saved_layouts as $key => $item) {
				...
				// Is this overwritten?
				$class = (in_array($key, $ancestor_keys)) ? 'disabled' : false;
				if (!$class && isset($_data['_plugin']['layouts'])) $class = (array_key_exists($key, $_data['_plugin']['layouts'])) ? 'disabled' : false ;
